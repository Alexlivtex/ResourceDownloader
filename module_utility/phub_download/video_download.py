...
def extract_link(url):
    ...
    for a in soup.find_all('a', href=True):
        if len(a['href'].split("viewkey=")) > 1:
            ...
            if not hash_value in total_has_list:
                total_has_list.append(hash_value)
                if len(total_has_list) % 50 == 0:
                    f_hash_total = open(pickle_data, "wb")
                    pickle.dump(total_has_list, f_hash_total)
                    f_hash_total.close()
...
def begin_hub_download():
    ...
    for hash_index in total_has_list:
